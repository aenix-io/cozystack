...
if [[ $ENABLE_SSL = "true" ]];then
  ...
  exist=$(kubectl get secret -n cozy-kubeovn kube-ovn-tls --ignore-not-found)
  if [[ $exist == "" ]];then
    docker run --rm -v "$PWD":/etc/ovn $REGISTRY/kube-ovn:$VERSION bash generate-ssl.sh
    kubectl create secret generic -n cozy-kubeovn kube-ovn-tls --from-file=cacert=cacert.pem --from-file=cert=ovn-cert.pem --from-file=key=ovn-privkey.pem
...
kubectl rollout status deployment/ovn-central -n cozy-kubeovn --timeout 300s
kubectl rollout status daemonset/ovs-ovn -n cozy-kubeovn --timeout 120s
echo "-------------------------------"
echo ""
...
kubectl rollout status deployment/kube-ovn-controller -n cozy-kubeovn --timeout 300s
kubectl rollout status daemonset/kube-ovn-cni -n cozy-kubeovn --timeout 300s
echo "-------------------------------"
echo ""
